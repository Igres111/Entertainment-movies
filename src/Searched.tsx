...
function Searched() {
  const { search, handleClickMark, movies } = useContext(GlobalAPI);
  const found = movies.filter((el) =>
    el.title.toLowerCase().includes(search.toLowerCase())
  );
  return (
    <div className="min-h-screen bg-hole">
      <div className="ml-4 text-white flex flex-wrap bg-hole ">
        <h1 className="  text-xl h-[25px] mt-[26px] mb-6	">
          Found {found.length} results for ‘{search}’
        </h1>
